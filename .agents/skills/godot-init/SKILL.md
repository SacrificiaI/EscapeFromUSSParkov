...
name: godot-init
description: >
  Initialize Codex for a Godot 4.x C# project: detect the existing
  project and its conventions, ask an ordered architecture questionnaire,
  derive an architecture from the answers, write a customized AGENTS.md, and
...
`AGENTS.md` at that project's root, written from what the project actually
...
3. **Generate** — compose and write `AGENTS.md` (Section 3).
...
folders/scenes/scripts, write `.Codex/rules/` or `.Codex/knowledge/`,
...
- **A `AGENTS.md` already exists at that root** → confirm the target with
...
- **Greenfield** — 0 hand-written `.cs` files, **or** ≤3 `.cs` and ≤2
  `.tscn` and no `.Codex/rules/`. Skip 0.5; the questionnaire answers
...
**If `.Codex/rules/*.md` already exists, read all of it first.** Those
...
Put the hit counts in the **report** (Section 4.3), not in `AGENTS.md`. The
...
- **Trust-boundary validation emphasis** — scales with how many "yes"/
  ...
  validation doctrine in the generated AGENTS.md entirely — there's no
  boundary to defend.
...
A `AGENTS.md` states **decisions and locations**, not explanations. Every
...
### 3.2 Handling an existing AGENTS.md
...
it silently), or **write alongside** as `Codex.generated.md` for manual
merge. Merge is the default offer.

### 3.3 The template

The literal skeleton lives in the sibling file
[`templates/AGENTS.md.template`](templates/AGENTS.md.template) in this same
...
`.Codex/rules/` or `.Codex/knowledge/` this skill finds already in place
...
Never silently overwrite an existing `AGENTS.md` — see 3.2.
...
AGENTS.md generation cannot break a build, so the build is not what proves
...
| Compile errors | Count, plus the first three `error CS` lines. State they are pre-existing and that AGENTS.md was still written. |
...
Detected    {{greenfield | existing}} — Godot {{version}}, SDK {{version}}, {{TFM}}
            {{N}} C# files, {{M}} scenes{{, existing .Codex/rules/}}
...
**Done** looks like: `AGENTS.md` exists at the project root, contains no
