...
description: >
  ...
  completed work, pending tasks, and learnings into .Codex/handoff.md, and
...
- **Session END** — capture exactly three things: what was DONE, what is
  PENDING, and what was LEARNED. Write them to `.Codex/handoff.md`.
...
Codex's built-in auto memory already does the "remember a correction
...
2. **Check uncommitted changes** — if any exist, offer to commit before
   wrapping.
3. **Write the handoff** — `.Codex/handoff.md`, using the format below.
...
4. **Extract learnings** — if the user corrected something or a non-obvious
   discovery surfaced (see checklist below), say so and let Codex's
...
### Handoff File Format (`.Codex/handoff.md`)
...
- [x] Fixed the Fix64/lockstep contradiction in .Codex/rules/doctrine.md
...
3. Did we decide something with rationale that isn't already an ADR? →
   consider whether it belongs in `.Codex/knowledge/decisions/` instead of
   only the handoff.
...
1. **Load context** — read `.Codex/handoff.md` (pending work) if it
...
Codex: Wrapping up session...

## Session Summary
- Extracted a Die() cleanup step in EnemyBase
- Fixed the Fix64/lockstep contradiction in .Codex/rules/doctrine.md

## Pending
- [ ] Confirm Squid and Devourer both still call it correctly

Written to .Codex/handoff.md. See you next session.
```
